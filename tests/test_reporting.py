...
def test_latest_signal_report_repository_source_filters_by_timeframe(
    ...
    assert report_contract_guard_checks["required_sections_present"][
        ...
        section["title"]
        for section in payload["sections"]
        if section["title"] in report_intent_contract["required_sections"]
    ]
    ...
    assert report_contract_guard_checks["intent_required_sections_present"][
        ...
        title for title in report_intent_contract["required_sections"]
    ...
    emitted_text_section_positions = {
        title: payload["text"].find(f"{title}:")
        for title in report_intent_contract["required_sections"]
    }
    assert report_contract_guard_checks["report_text_sections_match_intent_order"][
        "actual"
    ] == {
        "found_sections": [
            title for title in report_intent_contract["required_sections"]
            if emitted_text_section_positions[title] >= 0
        ],
        "positions_ascending": (
            list(emitted_text_section_positions.values())
            == sorted(emitted_text_section_positions.values())
        ),
        "positions": emitted_text_section_positions,
    }
    assert all(
    ...
    assert report_contract_guard_checks[
        ...
        report_contract_guard_checks["delivery_contract_keys_match_expected_schema"][
            "actual"
        ]
        == report_contract_guard_checks["delivery_contract_keys_match_expected_schema"][
            "expected"
    ...
    assert report_contract_guard_checks[
        ...
        report_contract_guard_checks["delivery_channel_formats_match_expected"][
            "actual"
        ]
        == report_contract_guard_checks["delivery_channel_formats_match_expected"][
            "expected"
    ...
    report_contract_guard_key_schema = report_contract_guard_checks[
        "report_contract_guard_keys_match_expected_schema"
    ]
    assert report_contract_guard_key_schema["expected"] == list(
        payload["report_contract_guard"]
    )
    assert report_contract_guard_key_schema["actual"] == list(
        payload["report_contract_guard"]
    )
    assert (
        report_contract_guard_key_schema["expected"]
        == report_contract_guard_key_schema["actual"]
    )
    assert all(
        str(database_path) not in value
        and not value.startswith("/")
        ...
        and ".sqlite3" not in value.lower()
        and not value.lower().startswith("sqlite:")
        for value in iter_nested_strings(report_contract_guard_key_schema)
    )
    assert report_contract_guard_checks[
        "report_contract_guard_keys_match_expected_schema"
    ]["passed"] == (
    ...
    report_contract_guard_check_keys = report_contract_guard_checks[
        "report_contract_guard_check_keys_match_expected_schema"
    ]
    assert (
        report_contract_guard_check_keys["expected"]
        == report_contract_guard_check_keys["actual"]
    )
    assert report_contract_guard_check_keys["actual"]["special_check_keys"] == {
        "telegram_text_fits_single_message": (
            emitted_report_contract_guard_check_keys[
                "telegram_text_fits_single_message"
            ]
    ...
    assert all(
        ...
        and ".sqlite3" not in value.lower()
        and not value.lower().startswith("sqlite:")
        for value in iter_nested_strings(report_contract_guard_check_keys)
    )
    assert report_contract_guard_checks[
        "report_contract_guard_check_keys_match_expected_schema"
    ]["passed"] == (
    ...
    source_repository_ref_guard_schema_targets = {
        "repository_ref_keys_guard_schema": [
            "report_payload_source_repository_ref_keys_match_expected_schema"
        ...
        "repository_ref_path_free_guard_schema": [
            "report_payload_source_repository_ref_is_path_free"
            in report_payload_guard_checks[
                "report_payload_guard_check_names_match_expected_schema"
            ]["expected"],
            ...
            report_payload_guard_checks[
                "report_payload_guard_check_keys_match_expected_schema"
            ]["actual"]["report_payload_source_repository_ref_is_path_free"]
            == ["name", "passed", "expected", "actual"],
        ],
    }
    source_repository_ref_guard_schema_summary = {
        name: all(checks)
        for name, checks in source_repository_ref_guard_schema_targets.items()
    }
    assert source_repository_ref_guard_schema_summary == {
        name: True for name in source_repository_ref_guard_schema_targets
    }
    assert report_payload_guard_checks[
        "report_payload_source_signal_ref_keys_match_expected_schema"
    ] == {
    ...
    assert str(database_path) not in iter_nested_strings(payload["sections"])
    assert str(database_path) not in iter_nested_strings(reasons)
    assert str(database_path) not in payload["text"]
    ...
    filtered_report_intent_surface_names = (
        "prompt_contract",
        "report_intent_contract",
    )
    ...
    filtered_report_source_surface_names = (
        "source_signal_ref",
        "source_repository_ref",
        "latest_record_guard",
    )
    assert {
        ...
        for name in filtered_report_source_surface_names
    } == {
        "source_signal_ref": len(iter_nested_strings(payload["source_signal_ref"])),
        "source_repository_ref": len(
            iter_nested_strings(payload["source_repository_ref"])
        ),
        "latest_record_guard": len(iter_nested_strings(latest_record_guard)),
    }
    assert {
        name: filtered_report_path_free_surface_string_counts[name]
    ...
    assert {
        ...
        "prompt_contract": len(iter_nested_strings(prompt_contract)),
        "report_intent_contract": len(iter_nested_strings(report_intent_contract)),
    }
    assert {
        name: filtered_report_path_free_surface_string_counts[name]
    ...
    assert {
        name: filtered_report_path_free_summary[name]
        for name in filtered_report_source_surface_names
    } == {name: True for name in filtered_report_source_surface_names}
    ...
    label_summary_hermes_boundary_tokens = [
        selected_label_summary,
        *excluded_label_summaries,
        selected_label["signal_id"],
    ...
    label_summary_hermes_boundary_summary = {
        name: all(
            token not in value
            for value in iter_nested_strings(target)
            for token in label_summary_hermes_boundary_tokens
        )
        for name, target in label_summary_hermes_boundary_targets.items()
    }
    ...
    selected_label_status_propagation_targets = {
        ...
        "evidence_context_label_status": [
            evidence_label_status[field] == selected_label[field]
            for field in (
                "signal_id",
                "outcome",
                "realized_r",
    ...
    source_summary_hermes_boundary_tokens = [
        ...
        "fits_single_message",
        "nonempty_chunk_text",
        "declared_separator_reconstructed_text",
        "reconstructed_text",
        "len(chunk.text)",
        "preview_keys",
        "channel_names",
        ...
        "expected_max_chars",
        "actual_chars",
        "continuation_chunks_start_with_required_section",
        "continuation_chunk_starts",
        "indexes",
        "text_len",
        "text_empty",
    ...
    selected_source_repository_ref_propagation_targets = {
        ...
        "evidence_context_source_repository_ref": [
            evidence_context["source_repository_ref"] == source_repository_ref,
        ],
        ...
        "report_payload_source_repository_ref_schema_guard": [
            ...
            == list(payload["source_repository_ref"]),
        ],
        ...
        "evidence_guard_source_repository_ref_schema_guard": [
            ...
            == list(evidence_context["source_repository_ref"]),
        ],
    ...
    degradation_hermes_boundary_summary = {
        name: all(
            token not in value
            for value in iter_nested_strings(target)
            for token in degradation_hermes_boundary_tokens
        )
        for name, target in degradation_hermes_boundary_targets.items()
    }
    ...
    conflict_flag_hermes_boundary_summary = {
        name: all(
            token not in value
            for value in iter_nested_strings(target)
            for token in conflict_flag_hermes_boundary_tokens
        )
        for name, target in conflict_flag_hermes_boundary_targets.items()
    }
