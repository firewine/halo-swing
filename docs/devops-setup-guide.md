...
whether `.env.example` and repo-root `.env` exist without writing files or
returning secret values. It also includes `live_data_smoke_commands` for `get_market_snapshot`,
...
`dotenv_file_status` but never secret values. Use
...
includes `live_data_setup_summary` and a provider route summary, then fails the
