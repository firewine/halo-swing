...
`provider_setup_actions`, `provider_setup_action_count`, and
`next_smoke_command_name` so the blocked stage and next provider setup action
are visible without returning secret values.
