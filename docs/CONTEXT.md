...
초기 목표는 **판단 보조, Hermes/Telegram용 delivery preview 리포트,
신호 기록, 사후 검증**이다.
리포트 도구는 Telegram message chunk와 Hermes payload metadata를 만들지만,
Telegram credential을 요구하거나 send call을 실행하지 않는다.
현재 주문 실행 표면은 BTC COIN-M에 한정된 confirmation-gated 도구만 있으며,
레버리지 ETF 주문과 범용 broker 자동화는 MVP 범위가 아니다.
...
Halo Swing MCP
  ...
  - 스윙 신호 계산
  - 매수/손절/익절 가이드 생성
  - Hermes/Telegram delivery preview payload 생성
  - 신호 기록과 결과 라벨링
  - 스코어링 피드백 계산
```
...
- 레버리지 ETF 주문과 범용 broker/order expansion은 MVP 범위가 아니다.
- BTC COIN-M order execution만 별도 guarded surface로 존재한다.
- BTC COIN-M 실행은 명시적 확인문구, live-trading flag, 암호화 credential,
  수동 passphrase, risk check 통과 없이는 동작하지 않는다.
- Hermes runtime start, Telegram send, scheduler 기반 자동 실행은 이후 gate
  전까지 제품 범위로 보지 않는다.
