...
```yaml
mode: implement
status: P1_REPOSITORY_SQLITE_LATEST_REPORT_FILTERED_SOURCE_SOURCE_REPOSITORY_REF_EXCLUDED_LABEL_SUMMARY_FREE_VALUE_VERIFIED
...
objective: extend SQLite filtered latest report coverage proving source_repository_ref remains excluded label summary-free after repository selection
...
done_when:
  - SQLite repository-backed latest report timeframe source_repository_ref excluded label summary-free values are verified after repository selection
  - SQLite repository-backed latest report underlying source_repository_ref excluded label summary-free values are verified after repository selection
  - source_repository_ref remains free of excluded label summaries outside the selected target summary
...
results:
  ...
  - full pytest passed: 935 passed in 55.62s
  - ruff passed
