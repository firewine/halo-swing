...
```yaml
mode: implement
status: P1_REPOSITORY_SQLITE_LATEST_REPORT_FILTERED_SOURCE_SELECTED_LABEL_STATUS_PROPAGATION_DIRECT_STATUS_MIRROR_VERIFIED
...
objective: extend SQLite filtered latest report coverage proving selected label status propagation summary mirrors direct surface status after repository selection
...
done_when:
  - SQLite repository-backed latest report timeframe selected label status propagation summary mirrors direct surface status after repository selection
  - SQLite repository-backed latest report underlying selected label status propagation summary mirrors direct surface status after repository selection
  - selected label status propagation aggregate remains aligned with direct per-surface booleans
...
results:
  ...
  - full pytest passed: 935 passed in 83.03s
  - ruff passed
