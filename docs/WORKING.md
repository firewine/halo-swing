...
```yaml
mode: implement
status: P1_REPOSITORY_SQLITE_LATEST_REPORT_FILTERED_SOURCE_SELECTED_LABEL_LATEST_MATCHING_EXCLUSION_TARGET_ORDER_VERIFIED
...
objective: extend SQLite filtered latest report coverage proving selected label latest-matching exclusion target order after repository selection
...
done_when:
  - SQLite repository-backed latest report timeframe selected label latest-matching exclusion target order is verified after repository selection
  - SQLite repository-backed latest report underlying selected label latest-matching exclusion target order is verified after repository selection
  - selected label latest-matching exclusion targets preserve filtered report target order after repository selection
...
results:
  ...
  - full pytest passed: 935 passed in 39.89s
  - ruff passed
