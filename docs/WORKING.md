...
```yaml
mode: implement
status: P1_REPOSITORY_SQLITE_LATEST_REPORT_FILTERED_SOURCE_SELECTED_FILTER_CONTRACT_RAW_MARKER_FREE_TARGET_ORDER_VERIFIED
...
objective: extend SQLite filtered latest report coverage proving selected filter contract raw marker free target order after repository selection
...
done_when:
  - SQLite repository-backed latest report timeframe selected filter contract raw marker free target order is verified after repository selection
  - SQLite repository-backed latest report underlying selected filter contract raw marker free target order is verified after repository selection
  - selected filter contract raw marker free targets preserve filtered report target order after repository selection
