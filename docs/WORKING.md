...
```yaml
mode: implement
status: P1_REPOSITORY_SQLITE_LATEST_REPORT_FILTERED_SOURCE_SELECTED_TRADE_PLAN_PRESENCE_TOKEN_ORDER_VERIFIED
gate_id: P1_REPOSITORY_SQLITE_LATEST_REPORT_FILTERED_SOURCE_SELECTED_TRADE_PLAN_PRESENCE_TOKEN_ORDER_GATE
review_tier: S1_small

objective: extend SQLite filtered latest report coverage proving selected trade plan presence token order after repository selection
...
done_when:
  - SQLite repository-backed latest report timeframe selected trade plan presence token order is verified after repository selection
  - SQLite repository-backed latest report underlying selected trade plan presence token order is verified after repository selection
  - selected trade plan presence checks preserve latest report entry, stop, take_profit, invalidation and rendered entry, stop, take_profit token order after repository selection
